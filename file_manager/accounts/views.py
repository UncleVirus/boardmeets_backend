...
class Register(APIView):
    ...
    def post(self,request,format=None):
        ...
        if user_data.is_valid():
            ...
            board_link= '<a href="http://172.98.72.70:4000/#/login-page" target="_blank" rel="noopener noreferrer">here</a>'
            html_content = f'<p>WELCOME TO BOARDMEET </br> <strong>Usname:</strong>{email} </br> <strong>Usname:</strong>{request.data["password"]} </br> <strong>Click:</strong>{board_link} to login  </p>'
...
class ForgotPassword(APIView):
    def post(self,request,format=None):
        try:
            ...
            reset_link = '127.0.0.1:8000/resetlostpassword/'
            reset_code = random.randint(0000, 10000)

            RestToken.objects.create(user=user,token=reset_code)

            html_message = f"<p>Reset Link, {reset_link}, Use this code to reset your password {reset_code}</p>"
            email = user.email
            message = Mail(
                from_email=settings.EMAIL_HOST_USER, 
                to_emails=email, 
